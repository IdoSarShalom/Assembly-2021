...
.data
	...
	adFlag db 0h ;adFlag = 1 ---> no ads, adFlag = 0 ---> with ads
	;**********************************************************************
.stack 100h

.code	
START:
	; ds points to the beginning of the Program Segment Prefix (PSP)
	
	mov al,ds:[80h] ;ax stores the input string length

	xor bl,bl ; reset the bl (flag) value, adFlag = 1 ---> no ads, adFlag = 0 ---> with ads
...
ExitGame:
	...
	cli ; block interrupts
	mov ax, ds:[Old_int_off]
	mov es:[8h*4], ax
	mov ax, ds:[Old_int_seg]
	mov es:[8h*4+2], ax
	sti ; set interrupts
